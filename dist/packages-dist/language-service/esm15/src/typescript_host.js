...
export class TypeScriptServiceHost {
    ...
    get resolver() {
        ...
        if (!result) {
            ...
            result = this._resolver = new CompileMetadataResolver(config, moduleResolver, directiveResolver, pipeResolver, new JitSummaryResolver(), elementSchemaRegistry, directiveNormalizer, new Console(), this._staticSymbolCache, this.reflector, (error, type) => this.collectError(error, type && type.filePath));
        }
        return result;
    }
    ...
    get staticSymbolResolver() {
        ...
        if (!result) {
            this._summaryResolver = new AotSummaryResolver({
                ...
                getOutputFileName(sourceFilePath) { return sourceFilePath; }
